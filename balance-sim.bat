@echo off
rem Headless balance run: the AutoPlayer plays to the ending (or N generations) at a fixed seed and writes
rem TestResults\balance.csv and balance.txt (year table + the S9 target summary), then prints them.
rem Usage: balance-sim.bat [seed] [generations]   generations 0 (default) = play to the Golden Year ending.
