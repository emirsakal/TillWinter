...
namespace TillWinter.Core.Balance
{
    ...
    /// weight/cost purchases, retire at a seed target, then greedy Heritage. Taps crows and clouds.
    /// </summary>
    public sealed class AutoPlayer
    {
        public float ReactionDelay = 0.3f;
        public float MaxRingSpeed = 6f;
        public int RetireAtSeeds = 10;
        ...
        public Dictionary<string, double> Weights = new Dictionary<string, double>
        {
            ...
            ["ring_combo"] = 0.5, ["late_frost"] = 0.6, ["frost_warning"] = 0.3, ["helper_water"] = 0.7, ["spring_head_start"] = 0.7,
        ...
        private void TickYear()
        {
            ...
            if (_sinceDecision >= ReactionDelay)
            {
                _sinceDecision = 0f;
                _target = PickTarget();
                foreach (var pos in new List<GridPos>(_crowSeen.Keys))
                {
        ...
        private void DoWinter()
        {
            ...
            _boughtThisWinter += BuyGreedy(Sim.Nodes);
            if (Sim.CanRetire && Sim.SeedsIfRetiredNow >= RetireAtSeeds)
            {
                Sim.Retire();
                return;
            }
            _boughtThisWinter += BuyGreedy(Sim.HeritageNodes);
            Sim.StartNextYear();
            StartRow();
        }
        ...
        private int BuyGreedy(IReadOnlyList<SkillNode> nodes)
        {
            int bought = 0;
            for (int guard = 0; guard < 200; guard++)
            {
                ...
                if (best == null || !Sim.TryBuy(best.Id)) break;
                bought++;
            }
            return bought;
        }
        ...
        public string ToCsv()
        {
            ...
            sb.AppendLine("generation,year,coins_year,coins_total,seeds,nodes_bought,apprentices,field,ring_radius,top_tier,first_ripe_s,harvests,share_ring,share_apprentice,share_tractor,share_late_frost");
            foreach (var r in Rows)
            {
                ...
                sb.AppendLine(string.Join(",", new[]
                {
                    ...
                    P(r.HarvestsRing, h), P(r.HarvestsApprentice, h), P(r.HarvestsTractor, h), P(r.HarvestsLateFrost, h),
                }));
            }
            return sb.ToString();
        }
